  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <div className="flex items-center gap-2 flex-1">
        <input type="color" value={value || placeholder} onChange={(e) => onChange(e.target.value)} className="w-8 h-8 rounded border border-slate-300 cursor-pointer" />
        <span className="text-xs text-slate-500 font-mono">{value || placeholder}</span>
        {value && (
          <button onClick={onClear} className="text-xs text-slate-400 hover:text-slate-600 ml-1" title="Reset to default">reset</button>
        )}
      </div>
    </div>
  );
...
export default function LeashBuddyCustomizer({ customizations, defaultColors, onUpdate, onRegenerate, isRegenerating }: LeashBuddyCustomizerProps) {
  ...
  return (
    <div className="glass-solid overflow-hidden border border-slate-200/50">
      <button onClick={() => setIsExpanded(!isExpanded)} className="w-full px-4 py-3 flex items-center justify-between hover:bg-slate-50/50 transition-colors">
      ...
      {isExpanded && (
        <div className="px-4 pb-4 space-y-5 border-t border-slate-200/50 pt-4">
          <section>
            <h4 className="text-sm font-semibold text-slate-900 mb-2">Material</h4>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {MATERIAL_OPTIONS.map((mat) => (
                <button key={mat.id} onClick={() => onUpdate({ material: mat.id })} className={`p-3 rounded-lg text-left transition-all border ${customizations.material === mat.id ? 'border-[var(--primary)] bg-[var(--primary)]/5' : 'border-slate-200 hover:border-slate-300'}`}>
          ...
          <section>
            <h4 className="text-sm font-semibold text-slate-900 mb-2">Ear Style</h4>
            <div className="grid grid-cols-4 gap-2">
              {EAR_STYLE_OPTIONS.map((ear) => (
                <button key={ear.id} onClick={() => onUpdate({ earStyle: ear.id })} className={`p-2 rounded-lg text-center transition-all border ${customizations.earStyle === ear.id ? 'border-[var(--primary)] bg-[var(--primary)]/5' : 'border-slate-200 hover:border-slate-300'}`}>
                  <p className="text-xs font-medium text-slate-900">{ear.label}</p>
                </button>
              ))}
            </div>
          </section>
          ...
          <section>
            <h4 className="text-sm font-semibold text-slate-900 mb-2">Ear Size</h4>
            <div className="grid grid-cols-3 gap-2">
              {EAR_SIZE_OPTIONS.map((size) => (
                <button key={size.id} onClick={() => onUpdate({ earSize: size.id })} className={`p-2 rounded-lg text-center transition-all border ${customizations.earSize === size.id ? 'border-[var(--primary)] bg-[var(--primary)]/5' : 'border-slate-200 hover:border-slate-300'}`}>
                  <p className="text-sm font-medium text-slate-900">{size.label}</p>
          ...
          <section>
            <label className="flex items-center gap-3 cursor-pointer">
              <input type="checkbox" checked={customizations.includeNameEmbroidery} onChange={(e) => onUpdate({ includeNameEmbroidery: e.target.checked })} className="w-4 h-4 rounded border-slate-300 text-[var(--primary)] focus:ring-[var(--primary)]/20" />
              <span className="text-sm text-slate-600">Embroider dog&apos;s name on back panel</span>
            </label>
          </section>

          <button onClick={onRegenerate} disabled={isRegenerating} className={`w-full py-3 rounded-lg text-sm font-semibold transition-all ${isRegenerating ? 'bg-slate-200 text-slate-400 cursor-not-allowed' : 'bg-[var(--primary)] text-white hover:bg-[var(--primary-hover)] shadow-sm hover:shadow-md'}`}>
            {isRegenerating ? (
