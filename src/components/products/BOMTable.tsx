  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="text-base font-semibold text-stone-900">Bill of Materials</h3>
      ...
      <div className="space-y-3">
        {Object.entries(grouped).map(([category, categoryItems]) => (
          <div key={category}>
            <p className="text-xs font-semibold text-stone-500 uppercase tracking-wide mb-1.5">{category}</p>
              ...
              {categoryItems.map((item, idx) => (
                <div key={idx} className="flex items-start justify-between px-3 py-2">
                  <div className="min-w-0 flex-1">
                    <p className="text-sm text-stone-900 font-medium">{item.name}</p>
                    {item.specification && <p className="text-xs text-stone-500 mt-0.5">{item.specification}</p>}
                  </div>
                  <div className="text-right flex-shrink-0 ml-3">
                    <p className="text-sm text-stone-700">{item.quantity} {item.unit}</p>
                    {item.estimatedCostUSD !== undefined && <p className="text-xs text-stone-400">${(item.estimatedCostUSD * item.quantity).toFixed(2)}</p>}
                  </div>
                </div>
              ))}
            </div>
