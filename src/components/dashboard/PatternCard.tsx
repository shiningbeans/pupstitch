...
export default function PatternCard({ pattern, onDelete }: PatternCardProps) {
  ...
  return (
    <>
      <div className="glass-solid group overflow-hidden hover:shadow-lg hover:border-stone-200 transition-all duration-300">
        <div className="relative h-48 overflow-hidden bg-stone-50">
          ...
          <div className="absolute top-3 right-3 bg-white/90 backdrop-blur-sm rounded-full px-3 py-1 text-xs font-medium text-stone-700 border border-stone-100 shadow-sm">
            {pattern.analysisResult.detectedBreed.split('-').map((w) => w.charAt(0).toUpperCase() + w.slice(1)).join(' ')}
          </div>
        </div>

        <div className="p-5 flex flex-col">
          <h3 className="text-base font-bold text-stone-900 mb-1 line-clamp-2">{pattern.name}</h3>
          <p className="text-xs text-stone-400 mb-4">Created {formatDate(pattern.createdAt)}</p>

          <div className="flex items-center gap-2 mb-4">
            <div className="flex gap-0.5">
              {Array.from({ length: 3 }).map((_, i) => (
                <svg key={i} className={`w-3.5 h-3.5 ${i < stars ? 'text-[var(--highlight)]' : 'text-stone-200'}`} fill="currentColor" viewBox="0 0 20 20">
                  <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                </svg>
              ))}
            </div>
            <span className="text-xs text-stone-400 capitalize">{pattern.generatedPattern.skillLevel}</span>
          </div>

          <div className="grid grid-cols-2 gap-3 mb-5 py-3 px-3 bg-stone-50 rounded-lg">
            <div className="text-center">
              <div className="text-xl font-bold text-stone-900">{estimatedHours}</div>
              <div className="text-xs text-stone-400">{estimatedHours === 1 ? 'Hour' : 'Hours'}</div>
            </div>
            <div className="text-center">
              <div className="text-xl font-bold text-stone-900">{bodyPartCount}</div>
              <div className="text-xs text-stone-400">{bodyPartCount === 1 ? 'Part' : 'Parts'}</div>
            </div>
          </div>
