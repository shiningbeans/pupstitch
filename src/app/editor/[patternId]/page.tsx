...
export default function EditorPage() {
  ...
  const leashBuddyCustomizations = usePatternStore((s) => s.leashBuddyCustomizations);
  const updateLeashBuddyCustomizations = usePatternStore((s) => s.updateLeashBuddyCustomizations);
  ...
  // If pattern isn't in store memory, try loading from IndexedDB (once)
  useEffect(() => {
    if (pattern || loadAttempted.current) return;
    loadAttempted.current = true;

    console.log('[Editor] Pattern not in store, loading from IndexedDB for:', patternId);
    ...
    (async () => {
      try {
        ...
        if (!after.currentPattern || after.currentPattern.id !== patternId) {
          console.warn('[Editor] Pattern not found in IndexedDB:', patternId);
          setLoadFailed(true);
        } else {
  ...
  // If pattern arrives late (e.g., store update propagated), clear any failure
  useEffect(() => {
    if (pattern && loadFailed) {
      setLoadFailed(false);
    }
  }, [pattern, loadFailed]);
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      // If viewing LeashBuddy tab and spec exists, download tech spec
  ...
  // ─── RENDER ───────────────────────────────────────────────────────
  // Priority 1: Pattern available → show it immediately
  if (pattern) {
    ...
    return (
      <div className="min-h-screen bg-gradient-honey">
        ...
        {/* Product Tabs — shown when LeashBuddy spec exists */}
        {(leashBuddySpec || pattern.leashBuddySpec) && (
          <div className="max-w-7xl mx-auto px-4 pt-6 print:hidden">
            <div className="flex gap-1 bg-amber-50 p-1 rounded-xl border border-amber-200">
              <button
                onClick={() => setActiveTab('leash-buddy')}
                className={`flex-1 py-2.5 px-4 rounded-lg text-sm font-semibold transition-all ${
                  activeTab === 'leash-buddy'
        ...
        {activeTab === 'leash-buddy' && (leashBuddySpec || pattern.leashBuddySpec) ? (
          <div className="max-w-3xl mx-auto px-4 py-6 print:hidden">
            <LeashBuddySpecPanel
              ...
              onUpdateCustomizations={updateLeashBuddyCustomizations}
              onRegenerate={generateProductPreview}
            />
          </div>
        ) : (
          <>
            {/* Preview Image + Body Part Analysis — ALWAYS shown for PupStitch */}
            <div className="max-w-7xl mx-auto px-4 pt-6 print:hidden">
  ...
  // Priority 2: Load failed → show error
  if (loadFailed) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-honey">
        <div className="text-center max-w-md">
          ...
          <p className="text-warm-secondary mb-6">
            The pattern you are looking for could not be loaded.
          </p>
